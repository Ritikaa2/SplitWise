...
async function createSettlement(groupId, payload, actingUserId) {
  ...
  if (!(await isActiveMember(groupId, payload.payer_id, date)) || !(await isActiveMember(groupId, payload.payee_id, date))) {
  ...
  return (await listSettlements(groupId)).find((item) => item.id === Number(result.lastInsertRowid));
}
