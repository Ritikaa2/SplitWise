...
function dashboard(userId) {
  const groups = db.prepare("SELECT g.* FROM groups g JOIN group_members gm ON gm.group_id = g.id WHERE gm.user_id = ?")
    .all(Number(userId));
  ...
  for (const group of groups) {
    ...
    for (const expense of db.prepare("SELECT * FROM expenses WHERE group_id = ? ORDER BY date DESC").all(group.id)) {
  ...
  const budgets = groupIds.length
    ? db.prepare(`
        SELECT b.category, b.monthly_limit, b.currency, g.name AS group_name,
          COALESCE(SUM(CASE WHEN e.date >= date('now', 'start of month') THEN e.converted_amount_inr ELSE 0 END), 0) AS spent
        FROM budgets b
  ...
  return {
    ...
    pending_settlements: groups.reduce((sum, group) => sum + simplifyDebts(group.id).length, 0),
    ...
    category_breakdown: categoryBreakdown(groupIds),
...
function groupBalances(groupId) {
  ...
  return {
    balances: [...balances.entries()].map(([user_id, amount]) => {
      const user = db.prepare("SELECT name FROM users WHERE id = ?").get(user_id);
      return { user_id, user_name: user?.name || `User ${user_id}`, amount: round2(amount), currency: "INR" };
    }),
    settlement_plan: simplifyDebts(groupId),
  };
}

function auditFeed(userId) {
  return db.prepare(`
