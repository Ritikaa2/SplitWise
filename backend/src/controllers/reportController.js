...
exports.dashboard = async (req, res) => {
  try {
    ...
    const [amountSpent] = await pool.query(
      `SELECT COALESCE(SUM(e.converted_amount_inr), 0) as total
       FROM expenses e
       JOIN group_members gm ON gm.group_id = e.group_id
       WHERE e.paid_by_id = ? 
       AND gm.user_id = ?
    ...
    const [monthly] = await pool.query(
      `SELECT
          ...
          SUM(e.converted_amount_inr) as amount
       FROM expenses e
       JOIN group_members gm ON gm.group_id = e.group_id
       WHERE gm.user_id = ?
    ...
    const [categories] = await pool.query(
      `SELECT
          e.category as name,
          SUM(e.converted_amount_inr) as value
       FROM expenses e
       JOIN group_members gm ON gm.group_id = e.group_id
       WHERE gm.user_id = ?
       AND gm.left_at IS NULL
    ...
    const [recent] = await pool.query(
      `SELECT
          e.title,
          e.date,
          e.converted_amount_inr as amount,
          g.name as \`group\`,
          e.category
       FROM expenses e
