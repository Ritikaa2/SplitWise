      ...
      <aside className="glass sticky top-0 z-20 flex h-auto items-center justify-between gap-3 border-x-0 border-t-0 p-4 md:h-screen md:flex-col md:items-stretch md:border-y-0 md:border-l-0">
        <div>
          ...
          <nav className="flex gap-1 overflow-x-auto md:flex-col">
            {nav.map(({ to, label, icon: Icon }) => (<NavLink key={to} to={to} end={to === "/app"} className={({ isActive }) => `flex h-10 items-center gap-3 rounded-lg px-3 text-sm transition ${isActive ? "bg-primary text-white" : "text-slate-300 hover:bg-white/10"}`}>
