...
export default function Landing() {
    return (<div className="min-h-screen overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.1 }} className="relative">
          <Card className="p-4">
            ...
            <div className="grid gap-3">
              {["Aarav pays Meera INR 4,200", "Nina receives INR 8,900", "CSV import flagged 3 warnings"].map((item) => (<div key={item} className="flex items-center justify-between rounded-lg bg-slate-950/50 p-4">
      ...
      <section className="mx-auto grid max-w-7xl gap-4 px-5 pb-20 md:grid-cols-4">
        {features.map((feature, index) => (<Card key={feature}>
